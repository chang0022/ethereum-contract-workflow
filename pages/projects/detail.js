...
import { Grid, Button, Typography, LinearProgress, Paper, CircularProgress, TextField } from '@material-ui/core'
...
class ProjectDetail extends React.Component {
  ...
  async contributeProject() {
    ...
    // 字段合规检查
    if (amount <= 0) return this.setState({ errMsg: '投资金额必须大于0' })
    if (amount < minInvestInEther) return this.setState({ errMsg: '投资金额必须大于最小投资金额' })
    if (amount > maxInvestInEther) return this.setState({ errMsg: '投资金额必须小于最大投资金额' })
