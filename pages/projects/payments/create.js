...
class PaymentCreate extends React.Component {
  ...
  async createPayment() {
    ...
    // 字段合规检查
    if (!description) return this.setState({ errmsg: '支出理由不能为空' })
    if (amount <= 0) return this.setState({ errmsg: '支出金额必须大于0' })
    if (!web3.utils.isAddress(receiver)) return this.setState({ errmsg: '收款人账户地址不正确' })
